...
本プロジェクトは以下のオープンソースソフトウェアのマニフェストを含んでいます — ぜひ応援してください：

- **[Rocky Linux](https://rockylinux.org/)** — 本プロジェクトを実現する基盤。Red Hat Enterprise Linuxと100%バグ互換を目指すコミュニティエンタープライズOS。
- **[px4_drv](https://github.com/tsukumijima/px4_drv)** by [tsukumijima](https://github.com/tsukumijima)（原作：[nns779](https://github.com/nns779/px4_drv)）— PLEXおよびe-Better TVチューナー用Linuxドライバー。**GPL-2.0**ライセンス。
- **[EPGStation](https://github.com/l3tnun/EPGStation)** by [l3tnun](https://github.com/l3tnun) — デジタル放送録画システム
- **[Mirakurun](https://github.com/Chinachu/Mirakurun)** by [Chinachu](https://github.com/Chinachu) — デジタル放送チューナーサーバー
- **[Jellyfin](https://jellyfin.org/)** — フリーソフトウェアメディアシステム
- **[Tube Archivist](https://github.com/tubearchivist/tubearchivist)** by [bbilly1](https://github.com/bbilly1) — YouTubeアーカイブマネージャー
- **[Navidrome](https://github.com/navidrome/navidrome)** — モダンな音楽サーバー＆ストリーマー

サードパーティソフトウェアは各自のライセンスを保持しています。本プロジェクトのインストーラースクリプトとKubernetesマニフェストは特に記載がない限りMITライセンスで提供されます。

## AI利用方針

本リポジトリは[Fedora AI支援貢献ポリシー](https://docs.fedoraproject.org/en-US/council/policy/ai-contribution-policy/)に準拠しています。ClaudeのOpus 4.6モデルを使用してすべてを作成し、人間によるテストとフィードバックで検証しています。
...
**[2] 特定のモジュールを実行** — 全21モジュール（00〜20）を番号順に表示します。モジュール番号を入力して個別に実行します。1桁の入力も可能です（`5`は`05`と同じ）。完了済みモジュールは`(done)`と表示されます。
...
本リポジトリのインストーラースクリプトとKubernetesマニフェストは、特に記載がない限り[MITライセンス](../../LICENSE)で提供されます。サードパーティソフトウェア（px4_drv、EPGStation、Mirakurun、Jellyfin、Tube Archivist、Navidrome）は各自のライセンスを保持します — 詳細は各上流リポジトリを参照してください。

px4_drvドライバーは[GPL-2.0](https://github.com/tsukumijima/px4_drv/blob/develop/LICENSE)ライセンスです。本リポジトリのRPMパッケージングはこのライセンスを尊重し保持しています。
